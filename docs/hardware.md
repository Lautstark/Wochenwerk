...
That last clause was written on a Mac and is true only there. macOS ships PC/SC in the system, so the bridge really is nothing but a file. Linux does not: PC/SC on Debian is the `pcscd` daemon plus the `libccid` driver, and both have to be installed and running before the file has anything to talk to. The kernel makes it worse before it makes it better — `pn533_usb` and `nfc` recognise the ACR122U as an NFC device, claim it on sight, and lock pcscd out with no error anywhere, so they have to be blacklisted. What is genuinely free of packages and builds is the Python. The half that touches the reader is an installation, and it is written down in [wyse-einrichten.md](wyse-einrichten.md).

The same seam runs through the file itself. PC/SC was born on Windows, and the two ports disagree about how wide `LONG` is: Apple pinned it to `int32_t`, pcsc-lite took the C `long`, which on x86_64 is 64 bits. That decides `DWORD`, `SCARDCONTEXT`, `SCARDHANDLE` and the position of every field in `READERSTATE` — and the wrong width raises nothing at all, it returns rubbish. So the bridge chooses its library and its integer widths together, at import, from `sys.platform`, and writes every call signature out by hand instead of letting ctypes assume 32 bits the way it does by default.

And *reader offline* is the bridge's own exit rather than a message it sends. When there is no reader to watch, it stops; the stream breaks, and the board reads the break. A bridge that stayed up reporting no card would be asserting the opposite of what is happening — every answer withdrawn at once, which is the one failure that reaches the child. Whatever starts it is therefore also what restarts it.
