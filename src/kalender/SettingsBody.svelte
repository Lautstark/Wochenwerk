...
<details class="panel" name="settings"><summary><span class="section">Ablage</span><span class="state">{whereSays(where)}</span></summary><div class="panel__body"><Vanilla node={store.node} /></div></details>
<details class="panel" name="settings"><summary><span class="section">Sicherung</span><span class="state">{keepingState || "Nur von Hand"}</span></summary><div class="panel__body">{#if keepingPanel}<Vanilla node={keepingPanel.node} /><hr class="hair" />{/if}<p class="small muted">Eine Momentaufnahme. Sie altert — übersteht aber einen Fehler, den der Ordner sofort mitmacht.</p><div class="acts"><button class="btn sm" type="button" onclick={() => void run(async () => {
    ...
    }, ""))}>Sicherung einlesen</button></div><p class="small muted">Einlesen fügt hinzu und überschreibt nie.</p></div></details>
<details class="panel" name="settings"><summary><span class="section">Symbole</span><span class="state">{symbolsState || "Von ARASAAC"}</span></summary><div class="panel__body">{#if !ready}<p class="small">{connected ? "Für METACOM legst du deinen lizenzierten Ordner in die Ablage." : "Ohne eigenen Ordner kommen die Symbole von ARASAAC — ohne Einrichtung."}</p>{/if}{#if !ready && connected}<pre class="tree">{[folderName(where) || HOME, `├── METACOM_9_Desktop   ← hier hinein`, "├── termine", "└── personen"].join("\n")}</pre><p class="small muted">Meist heißt er METACOM_9_Desktop. Wer die Ablage teilt, teilt METACOM mit — ob das erlaubt ist, steht in deiner Lizenz.</p>{/if}{#if looked}<p class="notice bad">{looked.length ? `Dort ist kein METACOM-Ordner. Gefunden habe ich: ${looked.join(", ")}.` : "Der Ordner ist noch leer."}</p>{/if}{#if connected && !ready}<div class="acts"><button class="btn sm primary" type="button" onclick={() => void lookForMetacom()}>Nochmal nachsehen</button></div>{/if}<Vanilla node={symbolsPanel.node} />{#if ready}<p class="small">{fromFolder ? `METACOM liegt in „${folderName(where)}“ — jedes Gerät, das die Ablage erreicht, zeichnet damit.` : "Gezeichnet wird mit METACOM aus einem eigenen Ordner."}</p>{/if}{#if found.length >= 2}<div class="opt"><label class="field-row"><span class="lbl">Darstellung</span><Dropdown label={namedRendering(preferredRendering())} build={add => { const live = preferredRendering(); add(namedRendering(null), chooseRendering(null), { checked: live === null }); for (const entry of found) add(namedRendering(entry.segment), chooseRendering(entry.segment), { checked: live === entry.segment }); }} /></label><p class="small muted">METACOM führt dieselben Symbole mehrfach. Die Vorgabe sortiert die Suche; ausgeschlossen wird nichts.</p></div>{/if}</div></details>
<details class="panel" name="settings"><summary><span class="section">Stimme</span><span class="state">{!loaded ? "Wird geladen …" : chosen ? namedVoice || "gewählte Stimme fehlt" : "keine gewählt"}</span></summary><div class="panel__body"><p class="small muted">Eine Stimme für den ganzen Kalender — nicht je Termin oder Karte.</p>{#if refused}<p class="notice bad">Azure nimmt den Schlüssel nicht an ({refused}). Unten stehen nur die Stimmen, die keinen brauchen.</p>{/if}{#if chosen && !namedVoice}<p class="notice">Die gewählte Stimme gibt es auf diesem Gerät gerade nicht. Bis eine andere gewählt wird, bleibt sie gespeichert.</p>{/if}{#if loaded}{#if voices.length}<Vanilla node={picker.node} />{:else}<p class="empty">keine Stimme verfügbar</p>{/if}{/if}</div></details>
<details class="panel" name="settings"><summary><span class="section">Sprachdienst</span><span class="state">{!azureLoaded ? "Wird geladen …" : azure ? `Schlüssel ••••${azure.key.slice(-4)}` : "Kein Schlüssel"}</span></summary><div class="panel__body"><p class="small muted" role="status">{probe}</p><p class="small muted">Kostenpflichtig, braucht ein Konto bei Microsoft. Der Schlüssel bleibt in diesem Browser und geht direkt zu Microsoft.</p><p class="small muted">Ein Schlüssel für den ganzen Kalender.</p><div class="row-of"><label class="field-row"><span class="lbl">Schlüssel</span><input class="field" type="password" autocomplete="off" placeholder={azure ? `••••${azure.key.slice(-4)}` : ""} bind:value={key} /></label><label class="field-row"><span class="lbl">Region</span><input class="field" type="text" list="azure-regionen" spellcheck="false" bind:value={region} /></label></div><datalist id="azure-regionen">{#each AZURE_REGIONS as name}<option value={name}></option>{/each}</datalist><p class="small muted">Steht im Azure-Portal bei deiner Speech-Ressource.</p><div class="acts"><button class="btn sm primary" type="button" disabled={checking} onclick={() => void keep()}>{checking ? "Wird geprüft …" : "Speichern"}</button>{#if azure}<button class="btn sm destructive" type="button" onclick={() => void forgetKey()}>Schlüssel entfernen</button>{/if}</div></div></details>
<details class="panel" name="settings"><summary><span class="section">Karten</span><span class="state">{cardList.length} {cardList.length === 1 ? "Karte" : "Karten"}</span></summary><div class="panel__body">{#if editingCard}<CardEditor card={editingCard} done={async () => { editingCard = null; await load(); moved(); }} />{:else}<p class="small muted">Karten sind das, was zur Wahl steht: ein Bild mit NFC-Tag, das du hinlegst.</p><div class="rows">{#each cardList as card}<Row title={card.name}>{#snippet lead()}<Picture symbol={card.symbol} name={card.name} />{/snippet}{#snippet state()}{#if card.nfc}<code class="nfc">{card.nfc}</code>{:else}<span class="row__state small muted">keine Nummer</span>{/if}{/snippet}{#snippet actions()}<Overflow build={add => { add("Bearbeiten", () => { editingCard = card; }); add("Entfernen", () => void eraseCard(card), { danger: true }); }} />{/snippet}</Row>{/each}</div>{#if !cardList.length}<p class="empty">noch keine</p>{/if}<button class="btn sm" type="button" onclick={() => { editingCard = { id: uuid(), name: "", updatedAt: 0 }; }}>＋ Neue Karte</button>{/if}</div></details>
<details class="panel" name="settings"><summary><span class="section">Personen</span><span class="state">{people.length} {people.length === 1 ? "Person" : "Personen"}</span></summary><div class="panel__body">{#if editingPerson}<PersonEditor person={editingPerson} done={async () => { editingPerson = null; await load(); moved(); }} />{:else}<div class="rows">{#each people as person}<Row title={person.name} state={person.birthday ? `Geburtstag ${dayLabel(person.birthday)}` : "kein Geburtstag"}>{#snippet lead()}<Face {person} />{/snippet}{#snippet actions()}<Overflow build={add => { add("Bearbeiten", () => { editingPerson = person; }); add("Entfernen", () => void erasePerson(person), { danger: true }); }} />{/snippet}</Row>{/each}</div>{#if !people.length}<p class="empty">noch niemand</p>{/if}<button class="btn sm" type="button" onclick={() => { editingPerson = { id: uuid(), name: "", initials: "", tone: "", updatedAt: 0 }; }}>＋ Neue Person</button>{/if}</div></details>
<details class="panel" name="settings"><summary><span class="section">Aussehen</span><span class="state">{THEME_WORDS[theme]}</span></summary><div class="panel__body"><div class="segmented" role="group" aria-label="Aussehen">{#each THEMES as option}<button type="button" aria-pressed={option === theme} onclick={() => { saveTheme(THEME_KEY, option); applyTheme(option); theme = option; }}>{THEME_WORDS[option]}</button>{/each}</div><p class="small muted">Gilt für den Kalender in diesem Browser. Das Board bleibt dunkel.</p></div></details>
<details class="panel" name="settings"><summary><span class="section">Löschen</span><span class="state"></span></summary><div class="panel__body"><p class="small muted">Den Kalender leeren und von vorn planen. Karten und Personen bleiben.</p><div class="acts"><button class="btn sm" type="button" onclick={() => void wipe(false)}>Alle Termine löschen</button></div><hr class="hair" /><p class="small muted">Alles, was Wochenwerk kennt. Danach ist es wie frisch installiert.</p><div class="acts"><button class="btn sm destructive" type="button" onclick={() => void wipe(true)}>Alle Daten löschen</button></div></div></details>
